...
export default abstract class IdentityHelper {
    ...
    /**
     * Sets a photo for this identity
     * @param client - The Olvid client instance
     * @param filepath - Path to the image file to use as the identity photo
     * @returns Promise that resolves when the photo is set
     */
    public static async setPhoto(client: OlvidClient, filepath: string): Promise<void> {
        await client.identitySetPhoto({ filePath: filepath });
    }
