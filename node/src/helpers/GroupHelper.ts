...
export default abstract class GroupHelper {
     ...
     */
    public static async sendMessage(client: OlvidClient, group: Group, body: string, options?: {attachments?: {filename: string, payload: Uint8Array}[], disableLinkPreview?: boolean, ephemerality?: MessageEphemerality}) : Promise<Message> {
     ...
     */
    public static async setPhoto(client: OlvidClient, group: Group, filePath: string): Promise<void> {
        await client.groupSetPhoto({ groupId: group.id, filePath: filePath });
    }
