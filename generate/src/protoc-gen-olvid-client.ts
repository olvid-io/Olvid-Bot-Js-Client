        ...
        let wrappedEndCallback = (error ?: Error) => {
            if (error) {
                ...
                if (this.isConnectionError(error)) {
                    console.error("${methodName}: connection error, stopping client", error);
                    this.stop();
                    return;
                }
...
export function generateOlvidClient(schema: Schema, web: boolean) {
    ...
    let clientFile = schema.generateFile("AbstractOlvidClient.ts");
...
export function generateOlvidAdminClient(schema: Schema, web: boolean) {
    ...
    let clientFile = schema.generateFile("AbstractOlvidAdminClient.ts");
