...
class EnhancedMessage extends Message{
     ...
     */
    public async reply(client: OlvidClient, body: string, options?: {quote?: boolean, disableLinkPreview?: boolean, ephemerality?: MessageEphemerality}) : Promise<Message> {
    ...
    public on = {
        ...
        /**
         * Sets up a persistent listener for incoming messages replying to this message events
         * @param client - The Olvid client instance
         * @param handler - Callback function to execute when the message is edited
         * @returns Function to cancel the event listener
         */
        messageReplied: (client: OlvidClient, handler: (message: Message) => Promise<void> | void): Function => {
            return client.onMessageBodyUpdated({
                callback: handler,
